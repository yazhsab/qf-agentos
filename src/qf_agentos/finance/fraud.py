...
An honest experimentation harness — NOT a claim that quantum beats classical.
Trains strong classical baselines (logistic regression + RBF kernel-ridge) and a
quantum fidelity-kernel classifier on the SAME kernel-ridge learner, on a
temporal holdout, and reports whether any quantum improvement is statistically
significant (bootstrap CI). Capacity/leakage/temporal checks are deterministic.
"""
...
class FraudDetectionDomain(ClassificationDomain):
    ...
    def requirements(self, spec: ProblemSpec) -> RequirementsReport:
        ...
        if pos_rate < 0.1:
            gaps.append(
                f"Highly imbalanced ({pos_rate:.0%} positive) — accuracy is misleading; AUC preferred."
            )
        ...
        return RequirementsReport(
            ...
            summary=f"{n} samples x {d} features, {pos_rate:.0%} positive; metric={cfg.target_metric}",
            metrics={
                "n_samples": float(n),
                "n_features": float(d),
            ...
            assumptions=["Temporal holdout: the latest samples are the test set (no look-ahead)."],
            ...
            if model.kind == "quantum" and comparator is not None:
                ...
                rep.quantum_contribution = {
                    ...
                    "verdict": (
                        ...
                        else "No statistically significant quantum improvement over the classical kernel."
                    ),
                }
            reports[name] = rep
        return reports
